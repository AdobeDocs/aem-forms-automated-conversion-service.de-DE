---
title: Konfigurieren des Automated forms conversion-Dienstes (AFCS)
description: Bereiten Sie Ihre AEM-Instanz für die Verwendung des Automated forms conversion-Dienstes (AFCS) vor.
...
# Konfigurieren des Automated forms conversion-Dienstes (AFCS) {#about-this-help}

In dieser Hilfe wird beschrieben, wie ein AEM-Administrator den Automated forms conversion-Dienst (AFCS) konfigurieren kann, um die Konvertierung ihrer PDF forms in adaptive Formulare zu automatisieren. Diese Hilfe richtet sich an IT- und AEM-Administratoren in Ihrem Unternehmen. Benutzer dieser Hilfe sollten mit den folgenden Technologien vertraut sein:
...
Sie benötigen Folgendes, um den Automated forms conversion-Dienst (AFCS) zu verwenden:

* Automated forms conversion Service (AFCS) ist für Ihr Unternehmen aktiviert.
...
Der automated forms conversion-Dienst (AFCS) wird auf AEM Autoreninstanz ausgeführt. Sie benötigen AEM 6.4, AEM 6.5 oder AEM Forms as a Cloud Service, um eine AEM-Autoreninstanz einzurichten.
...
Nachdem Adobe den Zugriff für Ihr Unternehmen aktiviert und dem Administrator die erforderlichen Berechtigungen erteilt hat, kann sich der Administrator bei der Admin Console anmelden (detaillierte Anweisungen unten), ein Profil erstellen und Entwickler zum Profil hinzufügen. Entwickler können eine lokale Instanz von AEM Forms mit dem Automated forms conversion-Dienst (AFCS) in der Adobe Cloud verbinden.

Entwickler sind Mitglieder Ihrer Organisation, die den Konvertierungs-Service ausführen. Nur Entwickler, die zum AFCS-Profil (Adobe Automated forms conversion Service) hinzugefügt werden, sind berechtigt, den Automated forms conversion-Dienst (AFCS) zu verwenden. Führen Sie die folgenden Schritte aus, um ein Profil zu erstellen und ihm Entwickler hinzuzufügen. Es ist mindestens ein Profil erforderlich, um Entwicklern Ihres Unternehmens den erforderlichen Zugriff zu gewähren:

1. Melden Sie sich bei [Admin Console](https://adminconsole.adobe.com/) an. Verwendung **Adobe ID** des für die Anmeldung mit dem Automated forms conversion-Dienst (AFCS) bereitgestellten Administrators. Verwenden Sie keine andere ID oder Federated ID, um sich anzumelden.
...
### (Nur für Entwickler) Verbinden Ihrer lokalen AEM Forms-Instanz mit dem Automated forms conversion-Service (AFCS) unter Adobe Cloud {#connectafcadobeio}

Nachdem ein Administrator Ihnen Entwicklerzugriff gewährt hat, können Sie Ihre lokale AEM Forms-Instanz mit dem Automated forms conversion-Service (AFCS) verbinden, der auf der Adobe Cloud ausgeführt wird. Führen Sie die folgenden Schritte in der angegebenen Reihenfolge aus, um Ihre AEM Forms-Instanz mit dem Service zu verbinden:
...
Um den Automated forms conversion-Dienst (AFCS) zu verwenden, erstellen Sie ein Projekt und fügen Sie die API für den automatischen Forms-Konfigurationsdienst zum Projekt in der Adobe Developer-Konsole hinzu. Die Integration erzeugt API-Schlüssel, Client Secret, Payload (JWT).
...
1. Tippen **[!UICONTROL Dienstkonto (JWT)]** um den API-Schlüssel, das Client Secret und andere Informationen anzuzeigen, die zum Verbinden Ihrer lokalen AEM-Instanz mit dem Automated forms conversion-Service (AFCS) erforderlich sind. Die Informationen auf der Seite werden verwendet, um eine IMS-Konfiguration auf Ihrem lokalen Computer zu erstellen.
...
1. Geben Sie auf der Seite **[!UICONTROL Konfiguration für die automatisierte Formularkonvertierung erstellen]** Werte für die folgenden Felder ein und tippen Sie auf **[!UICONTROL Weiter]**.
   ...
   | Service-URL | URL des Automated forms conversion-Dienstes (AFCS) auf Adobe Cloud. Verwenden Sie die URL `https://aemformsconversion.adobe.io/`. |
